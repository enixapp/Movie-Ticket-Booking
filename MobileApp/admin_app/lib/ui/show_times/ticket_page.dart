...
import 'package:admin_app/domain/model/age_type.dart';
...
class TicketsPage extends StatefulWidget {
  ...
  const TicketsPage({Key? key, required this.showTime, required this.theatre})
      : super(key: key);
...
class _TicketsPageState extends State<TicketsPage> with DisposeBagMixin {
  ...
  void didChangeDependencies() {
    ...
    resBloc ??= LoaderBloc(
      loaderFunction: () => ticketRepository
          .getReservationsByShowTimeId(widget.showTime.id ?? ""),
      logger: print,
    )..fetch();
  }
...
class _SeatsGridWidgetState extends State<SeatsGridWidget> {
  ...
  Widget buildItem(BuildContext context, int y, int x, double widthPerSeat) {
    ...
    if (ticket == null) {
      int? prevCount;
      SeatCoordinates prevCoords = SeatCoordinates.from(x: 0, y: 0);
      var prevX = x - 1;
      while (prevX >= 0) {
...
class SeatWidget extends StatelessWidget {
  ...
  const SeatWidget(
      {Key? key, required this.ticket, this.tapTicket, this.widthPerSeat})
      : super(key: key);

  @override
  Widget build(BuildContext context) {
    final seat = ticket.seat;
    final width = (widthPerSeat ?? 0.0) * seat.count + (seat.count - 1) * 1;
...
class LegendsWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final taken = (tickets?.length ?? 0) - (avai ?? 0);
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                const SizedBox(width: 6),
                Text('${taken} Taken', style: textStyle),
              ],
            ),
            Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(5),
                    border:
                        Border.all(color: const Color(0xffCBD7E9), width: 1),
                  ),
                ),
                const SizedBox(width: 6),
                Text('Doubled seat', style: textStyle),
              ],
            ),
          ],
        ),
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(IterableProperty<Ticket>('tickets', tickets));
  }
}
          ...
          children: [
            ...
            RichText(
              text: TextSpan(text: 'Theatre: ', style: textStyle, children: [
                TextSpan(text: theatre.name, style: textStyle2),
                TextSpan(text: ' Room: ', style: textStyle),
                TextSpan(text: showTime.room, style: textStyle2),
              ]),
            ),
            const SizedBox(height: 8),
            RichText(
              text: TextSpan(text: 'Address: ', style: textStyle, children: [
                TextSpan(text: theatre.address, style: textStyle2),
              ]),
            ),
            const SizedBox(height: 6),
            const Padding(
  ...
  Widget build(BuildContext context) {
    return RxStreamBuilder<LoaderState<BuiltList<Reservation>>>(
      ...
      builder: (context, state) {
        ...
        final total = movies.fold(0, (acc, e) => (acc as int) + e.totalPrice);
        return SliverList(
