...
import 'package:rxdart_ext/rxdart_ext.dart' hide delay;
...
class _AppShowTimePageState extends State<AppShowTimePage>
          ...
          onPressed: () {
            ...
            showDialog<void>(
              ...
              builder: (BuildContext dialogContext) {
                return AlertDialog(
                  title: const Text('Available periods'),
        ...
        children: [
          ...
          Expanded(
            child: DateTimeField(
              ...
              resetIcon:
                  const Icon(Icons.delete, color: Colors.deepPurpleAccent),
  ...
  void submit() async {
    ...
    if (tickets == null) {
      context.showSnackBar('Invalid tickets');
      return;
    }
              ...
              children: [
                ...
                Expanded(
                  child: ProgressButton.icon(
                    ...
                    onPressed: submit,
                    state:
                        (snap /*?.data*/ as ButtonState?) ?? ButtonState.idle,
                    //state: snap as ButtonState? ?? ButtonState.idle,
                  ),
                ),
                const SizedBox(width: 32),
              ],
  ...
  Future<int?> pickPrice(int initPrice) async {
...
class SeatsGridWidget extends StatefulWidget {
  ...
  const SeatsGridWidget(
      {Key? key, required this.tickets, required this.tapTicket})
      : super(key: key);
...
class _SeatsGridWidgetState extends State<SeatsGridWidget> {
  ...
  Widget buildItem(BuildContext context, int y, int x, double widthPerSeat) {
    ...
    if (ticket == null) {
      int? prevCount;
      late SeatCoordinates prevCoords;
      var prevX = x - 1;
      while (prevX >= 0) {
