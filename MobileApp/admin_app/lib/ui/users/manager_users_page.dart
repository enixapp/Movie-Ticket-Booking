...
class ManagerUsersPage extends StatefulWidget {
  ...
  const ManagerUsersPage({Key? key}) : super(key: key);
  ...
  void initState() {
    ...
    _slidableController = SlidableController(
      ...
      onSlideIsOpenChanged: (isOpen) {
        setState(() => _isOpeningSlide = isOpen ?? false);
      },
    );
                    ...
                    return data[user.uid] == DestroyUserType.CHANGE_ROLE
                        ...
                        : IconSlideAction(
                            ...
                            onTap: () async {
                              ...
                              print(theatre);
                              _bloc?.destroyUser(
                                Tuple3(user, DestroyUserType.CHANGE_ROLE,
                                    theatre?.id ?? ""),
                              );
                            },
                          );
                  ...
                  builder: (context, index, animation, renderingMode) {
                    ...
                    final iconRemove = data[user.uid] == DestroyUserType.REMOVE
                        ...
                        : IconSlideAction(
                            ...
                            onTap: () async {
                              final isDismiss = await _showDialogConfirm(
                                  'Delete this user', 'User will be deleted ');
                              if (isDismiss != null && isDismiss) {
...
class UserItemWidget extends StatelessWidget {
  const UserItemWidget(this.user);
  ...
  Widget _buildStatusRoleUser(bool isActive) {
    return Row(
      ...
      children: [
        ...
        const Text('role: ', style: TextStyle(fontSize: 8)),
