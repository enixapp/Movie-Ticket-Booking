...
BuiltList<TheatreSeat> fullSeats() {
  final seats = <TheatreSeat>[];
...
class SeatsPage extends StatefulWidget {
  ...
  const SeatsPage({Key? key, required this.seats}) : super(key: key);
...
class _SeatsPageState extends State<SeatsPage> with DisposeBagMixin {
  late BehaviorSubject<BuiltList<TheatreSeat?>> changeSeatS;
  final longSelectedS = BehaviorSubject.seeded(<TheatreSeat?>{}.build());

  late DistinctValueStream<
      Tuple2<BuiltList<TheatreSeat>, BuiltSet<TheatreSeat?>>> seats$;
  ...
  void initState() {
    ...
    seats$ = Rx.combineLatest2(
      changeSeatS,
      longSelectedS,
      (a, b) => Tuple2<BuiltList<TheatreSeat>, BuiltSet<TheatreSeat>>(
          a as BuiltList<TheatreSeat>, b as BuiltSet<TheatreSeat>),
    ).shareValueDistinct(
      Tuple2(seedValue, longSelectedS.value as BuiltSet<TheatreSeat>),
      sync: true,
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            RxStreamBuilder<BuiltSet<TheatreSeat?>>(
          ...
          slivers: [
            ...
            const SliverToBoxAdapter(
              child: Padding(
                ...
                child: Divider(height: 1, color: Color(0xffD1DBE2)),
              ),
            ),
          ],
        ),
  ...
  void onMerge(BuildContext context) {
    ...
    final sorted = longSelected.sortedBy<num>((e) => e?.coordinates.x as int);
    print(sorted);
    for (var i = 0; i < sorted.length - 1; i++) {
      final cur = sorted[i];
      final after = sorted[i + 1];
      if ((after?.coordinates.x ?? 0) - (cur?.coordinates.x ?? 0) !=
          cur?.count) {
    ...
    longSelectedS.add(<TheatreSeat>{}.build());
    changeSeatS.add(seats);
  }
}

class SeatsGridWidget extends StatefulWidget {
  final Tuple2<BuiltList<TheatreSeat>, BuiltSet<TheatreSeat>> tuple;
  final Function1<TheatreSeat, void> changeSeat;
  final Function1<TheatreSeat, void> onLongPressed;
...
class _SeatsGridWidgetState extends State<SeatsGridWidget> {
  late int maxX;
  late int maxY;
  late Map<Coordinates, TheatreSeat> seatByCoordinates;
                    ...
                    return Container(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, col) {
                          return buildItem(context, row, col, widthPerSeat);
                        },
                      ),
                    );
                  },
  ...
  Widget buildItem(BuildContext context, int y, int x, double widthPerSeat) {
    ...
    int? prevCount;
    Coordinates cc = Coordinates(0, 0);
    if (seat == null) {
      var jj = x - 1;
      while (jj >= 0) {
        cc = Coordinates(jj, y);
        prevCount = seatByCoordinates[cc]?.count ?? 0;
        if (prevCount != null) {
          break;
        }
        jj--;
      ...
      return prevCount != null &&
          ...
          : InkWell(
              onTap: () => widget.changeSeat(TheatreSeat(row, 1, coordinates)),
              child: Container(
                margin: const EdgeInsets.all(0.5),
                width: widthPerSeat,
                height: widthPerSeat,
                decoration: BoxDecoration(
                  color: Colors.white,
                  borderRadius: BorderRadius.circular(5),
                  border: Border.all(color: const Color(0xffCBD7E9), width: 1),
                ),
              ),
            );
    } else {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(5),
          border: Border.all(
              color: isSelected ? accentColor : primaryColor, width: 1),
        ),
        child: Center(
          child: Text(
            '${seat.row}${column}',
