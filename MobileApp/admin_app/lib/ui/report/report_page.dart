...
class _ReportPageState extends State<ReportPage> with DisposeBagMixin {
  final BehaviorSubject<DateTime?> dayS =
      BehaviorSubject.seeded(DateTime.now(), sync: true);
  ...
  Widget buildCharts(BuiltMap<String, int> map, BuildContext context) {
    ...
    if (amount_sold == null || amount == null) {
      return const Center(child: Text("not any amount"));
    }
    final seriesList = [
    ...
    if (tickets_sold == null || tickets == null) {
      return const Center(child: Text("not any tickets"));
    }
    final seriesList2 = [
      charts.Series<Tuple2<String, int>, String>(
        id: 'Tickets',
